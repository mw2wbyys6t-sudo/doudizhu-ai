...
> **Terminology lock**: this file deals in **schools** (six high-level lenses) and **anchors** (named studios / brands / designers per school). The companion folder `style-recipe-*.md` contains 25 **recipe** files — one file per anchor — with concrete, ready-to-paste configurations. When a user picks a school here, hand them off to the recipe files in that school for concrete palette / typography / spacing values. Load only the recipe files you actually need; the catalog index is at `style-recipe-INDEX.md`.
...
5. **After the user picks a school → read 2–3 recipe files from `style-recipe-*.md` in that school** (e.g., picked *Information Architecture* → read `style-recipe-pentagram.md` + `style-recipe-bloomberg-terminal.md` + `style-recipe-tufte-dataink.md`). The recipe files carry the concrete values; this file does not duplicate them.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`pentagram`](./style-recipe-pentagram.md) · [`vignelli-swiss-helvetica`](./style-recipe-vignelli-swiss-helvetica.md) · [`bloomberg-terminal`](./style-recipe-bloomberg-terminal.md) · [`tufte-dataink`](./style-recipe-tufte-dataink.md) · [`nyt-the-daily`](./style-recipe-nyt-the-daily.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`apple-hig`](./style-recipe-apple-hig.md) · [`muji-kenya-hara`](./style-recipe-muji-kenya-hara.md) · [`aesop`](./style-recipe-aesop.md) · [`dieter-rams-braun`](./style-recipe-dieter-rams-braun.md) · [`monocle-magazine`](./style-recipe-monocle-magazine.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`field-io`](./style-recipe-field-io.md) · [`active-theory`](./style-recipe-active-theory.md) · [`resn-storytelling`](./style-recipe-resn-storytelling.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`are-na`](./style-recipe-are-na.md) · [`bloomberg-businessweek-turley`](./style-recipe-bloomberg-businessweek-turley.md) · [`balenciaga-post-2017`](./style-recipe-balenciaga-post-2017.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`mailchimp-freddie`](./style-recipe-mailchimp-freddie.md) · [`stripe-press`](./style-recipe-stripe-press.md) · [`headspace-meditation`](./style-recipe-headspace-meditation.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
**Concrete starting points** (each is a single file in `style-recipe-*.md` — read one): [`linear`](./style-recipe-linear.md) · [`vercel-mesh`](./style-recipe-vercel-mesh.md) · [`raycast`](./style-recipe-raycast.md) · [`notion-pre-ai`](./style-recipe-notion-pre-ai.md) — each carries the palette, typography, spacing, and signature moves to paste straight into Step 3.
...
> Each recipe file in `style-recipe-*.md` ships a pre-written **AI prompt seed** tuned to that recipe's DNA — start from the one you're using rather than writing prompts from scratch.
