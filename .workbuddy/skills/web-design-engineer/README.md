...
│       ├── style-recipe-INDEX.md          # Catalog index + 3 cross-indexes + cross-cutting anti-patterns
│       ├── style-recipe-linear.md         # One of 25 single-recipe files, loaded on demand
...
**Anchored style-recipe library (25 named recipes, progressively loaded).** When the user names an anchor ("Linear-style", "Aesop feeling", "Pentagram-grade type"), the agent reads **only the matching file** at `references/style-recipe-<anchor>.md` (~50 lines). The catalog index, 3 cross-indexes, and cross-cutting anti-patterns live in `references/style-recipe-INDEX.md` (~150 lines). The full catalog is never loaded at once. The 25 recipes are spread across 7 schools (the 6 Direction-Advisor schools plus a *Specialty / Genre* school reachable only via direct anchor names):
...
<br /><br />
<sub>See specs in <a href="./references/style-recipe-INDEX.md">style recipe index</a></sub>
<br /><br />
</td>
</tr>
</table>
